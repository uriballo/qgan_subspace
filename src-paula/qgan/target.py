...
def get_target_unitary(target_type: str, size: int, config=CFG) -> torch.Tensor:
...
def get_final_target_state(final_input_state: torch.Tensor, config=CFG) -> torch.Tensor:
    ...
    target_unitary = get_target_unitary(config.target_hamiltonian, config.system_size, config=CFG)
