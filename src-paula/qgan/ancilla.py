...
def get_max_entangled_state_with_ancilla_if_needed(size: int, config=CFG) -> tuple[torch.Tensor, torch.Tensor]:
    """
...
def get_final_gen_state_for_discriminator(total_output_state: torch.Tensor, config=CFG) -> torch.Tensor:
    ...
    if config.ancilla_mode == "pass":
        ...
        projected, _ = project_ancilla_zero(total_output_state, config=config)
        return projected
