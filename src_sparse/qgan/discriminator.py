...
class Discriminator:
    ...
    def __init__(self, config=CFG, seed=None):
        ...
        herm = [I, X, Y, Z]
        self.herm = [csr_matrix(h) for h in herm]
    ...
    def get_psi_and_phi(self) -> np.ndarray:
        ...
        psi, phi = 1, 1
        for i in range(self.size):
            psi_i, phi_i =  csr_matrix(self.herm[0].shape, dtype=complex), csr_matrix(self.herm[0].shape, dtype=complex)
            for j, herm_j in enumerate(self.herm):
                psi_i += self.alpha[i][j] * herm_j
                phi_i += self.beta[i][j] * herm_j
            psi, phi = kron(psi, psi_i), kron(phi, phi_i)
        return psi, phi
    ...
    def get_dis_matrices_rep(self) -> tuple:
        ...
        ##########################################################
        A = scipy.sparse.linalg.expm(float(-1 / lamb) * phi)
        B = scipy.sparse.linalg.expm(float(1 / lamb) * psi)

        return A.toarray(), B.toarray(), psi.toarray(), phi.toarray()
    ...
    def _grad_psi_or_phi(self, type: int, respect_to: str) -> list:
        """Calculate the gradient of the discriminator with respect to psi or phi (sparse).

        Args:
            type (int): The type of Hermitian operator (0: I, 1: X, 2: Y, 3: Z).
            respect_to (str): Specify whether to compute the gradient for psi or phi.

        Returns:
            list: A list of sparse gradients for each qubit in the discriminator.
        """
        coefficients = self.alpha if respect_to == "psi" else self.beta
        grad_matrix = []

        for i in range(self.size):
            grad_matrix_I = csr_matrix([[1]])  # Start with 1 as a 1x1 sparse matrix
            for j in range(self.size):
                if i == j:
                    grad_matrix_j = self.herm[type]  # Already assumed to be sparse
                else:
                    # Start with a sparse zero matrix of the same shape
                    grad_matrix_j = csr_matrix(self.herm[0].shape, dtype=complex)
                    for k, herm_k in enumerate(self.herm):
                        grad_matrix_j += coefficients[j][k] * herm_k
                grad_matrix_I = kron(grad_matrix_I, grad_matrix_j, format='csr')
