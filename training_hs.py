...
import numpy as np
from model_hs import Generator, Discriminator, compute_cost, compute_fidelity
from tools.plot_hub import plt_fidelity_vs_iter
from tools.qcircuit import *
import config_hs as cf
from tools.utils import save_model, train_log, get_maximally_entangled_state, get_maximally_entangled_state_in_subspace, get_zero_state
    ...
    # while (f < 0.95):
        ...
        for iter in range(cf.epochs):
            ...
            if iter % 10 ==0:
                ...
                param = 'epoches:{:4d} | fidelity:{:8f} | time:{:10s} | duration:{:8f}\n'.format(iter,round(fidelities[iter],6),time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),round(training_duration,2))
